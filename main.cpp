...
const double kmup = 0.00092;    // Half-saturation concentration of(mM)
...
double ina=0;    // Fast Na Current (uA/uF)
...
double ilca=0;    // Ca current through L-type Ca channel (uA/uF)
double ilcana=0;  // Na current through L-type Ca channel (uA/uF)
double ilcak=0;  // K current through L-type Ca channel (uA/uF)
...
double icat=0;    // Ca current through T-type Ca channel (uA/uF)
...
double ikr=0;   // Rapidly Activating K Current (uA/uF)
...
double iks=0;   // Slowly Activating K Current (uA/uF)
...
double iki=0;    // Time-independant K current (uA/uF)
...
double ikp=0;    // Plateau K current (uA/uF)
...
double ikna=0;   // Na activated K channel
...
double ikatp=0;    // ATP-sensitive K current (uA/uF)
...
double ito=0;	  // Transient outward current
...
double inaca=0;               // NaCa exchanger current (uA/uF)
...
double inak=0;    // NaK pump current (uA/uF)
...
double insna=0;     // Non-specific Na current (uA/uF)
...
/* Sarcolemmal Ca Pump */
double ipca=0;                 // Sarcolemmal Ca pump current (uA/uF)
...
void prttofile()
{
    ...
    if(t>(0) && t<(bcl*beats))
        fprintf(ap,"%.3f\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\t%g\n",
                t, v, nai, ki, cai, jsr, nsr, ina, ikr, iks, iki, ilca, icat, inab, icab, ipca, inaca, inak, ilcana, naiont, kiont, caiont, it, ilcak, ito, ikna, ikatp, cao,dcai,iup,ileak,dt);

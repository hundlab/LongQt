...
void dvarMenu::createMenu() {
  auto type = cell->type();
  QMap<QString, QString> definitions = GuiUtils::concatMaps(
  ...
  widgets.insert("Other", QList<QCheckBox*>());
  {
    ...
    for (i = 0; it != vars.end(); it++, i++) {
      ...
      connect(dvars[i], &QCheckBox::stateChanged,
              [=](int state) { update_datamap(p, state); });
      if (*it == "t" || *it == "vOld") {
  ...
  }
  {
    ...
    for (auto it = widgets.begin(); it != widgets.end(); it++) {
      ...
      std::sort(
          ...
          [](QCheckBox* a, QCheckBox* b) { return a->text() <= b->text(); });
      ...
      if (!it.value().isEmpty()) {
        central_layout->addWidget(box, currRow, 0, layout->rowCount(),
                                  layout->columnCount());
      }
  ...
  // main_layout
  main_layout->addLayout(central_layout, 1, 0, central_layout->rowCount(),
                         central_layout->columnCount());

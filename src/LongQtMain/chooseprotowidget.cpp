...
shared_ptr<Protocol> ChooseProtoWidget::getCurrentProto() { return proto; }
void ChooseProtoWidget::resetProto() { this->changeProto(0); }
...
void ChooseProtoWidget::changeProto(string name) {
  this->changeProto(shared_ptr<Protocol>(CellUtils::protoMap.at(name)()), true);
}
void ChooseProtoWidget::changeProto(shared_ptr<Protocol> newProto, bool raise) {
  if (newProto == this->proto) return;
...
void ChooseProtoWidget::on_cellType_currentIndexChanged(QString name) {
  this->proto->parsStr("celltype", name.toStdString());
...
void ChooseProtoWidget::changeCell(shared_ptr<Cell> cell) {
  if (cell != proto->cell()) {
    Logger::getInstance()->write(
        "ChooseProtoWidget: Protocol cell does not match new cell");
  }
...
void ChooseProtoWidget::on_readSettings_clicked() {
  QString fileName = QFileDialog::getOpenFileName(
      this, "Choose Settings file", proto->datadir.absolutePath());
  if (!fileName.isEmpty()) {
    SettingsIO* settingsMgr = SettingsIO::getInstance();
    this->proto = settingsMgr->readSettings(fileName, this->proto);
    emit protocolChanged(this->proto);
    emit cellChanged(proto->cell());
    this->updateMenu();
  }

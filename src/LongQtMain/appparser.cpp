...
AppParser::AppParser(QCoreApplication* app)
    : GUIOption("no-gui", QCoreApplication::translate(
                              "main", "Start LongQt in CLI mode")),
      licenseOption(
          QStringList() << "l"
                        << "license",
  ...
  parser.addPositionalArgument(
      ...
      QCoreApplication::translate(
          "main",
          "Specify files to read settings from. They will be processes "
          "serially in the order listed. If --no-gui is not set only the first "
          "file will be read."),
      "[simvars.xml...]");
}
...
void AppParser::start() {
  ...
  if (!startCLI) {
    ...
    if (settings.value("showHelp", true).toBool()) {
      ...
      if (QMessageBox::Discard ==
          QMessageBox::information(0, "Welcome!", showHelpText,

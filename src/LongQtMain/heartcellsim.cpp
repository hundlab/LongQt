...
Simulation::Simulation(QString simvarFile, QWidget* parent) {
  ...
  connect(choose, &ChooseProtoWidget::protocolChanged, this,
          &Simulation::changeProto);

  connect(choose, &ChooseProtoWidget::cellChanged, this,
          &Simulation::cellChanged);
  connect(this, &Simulation::cellChanged, choose,
          &ChooseProtoWidget::changeCell);
  connect(sims, &SimvarMenu::cellChanged, this, &Simulation::cellChanged);
  connect(this, &Simulation::working_dir_changed, run,
          &RunWidget::setWorkingDir);
  ...
  connect(
      run, static_cast<void (RunWidget::*)()>(&RunWidget::running), [this]() {
  ...
  menu_options->item(menu_options->count() - 1)
      ->setToolTip(
  ...
  menu_options->item(menu_options->count() - 1)
      ->setToolTip("Set model constants or have them randomly choosen");
  ...
  menu_options->item(menu_options->count() - 1)
      ->setToolTip(
          "Measure properties related to output variables (e.g. action "
  ...
  connect(menu_options, SIGNAL(currentRowChanged(int)), this,

...
public:
    explicit SimvarMenu(std::shared_ptr<LQ::Protocol> initial_proto, QWidget *parent = 0);
    void createMenu();
    ~SimvarMenu();
...
private:
    typedef std::function<Simvar*(std::shared_ptr<LQ::Protocol>, std::string)> SimvarInitializer;

...
ConductivityEditor::ConductivityEditor(QTableView* view, QWidget* parent)
    : QWidget(parent), ui(new Ui::ConductivityEditor) {
  ui->setupUi(this);
  this->view = view;
  this->model = (GridModel*)view->model();
}

ConductivityEditor::~ConductivityEditor() { delete ui; }
void ConductivityEditor::on_applyButton_clicked() {
  this->model->setPercent(ui->percValuesCheckBox->isChecked());
  this->setConductivities(ui->startingValueDoubleSpinBox->value(),
                          ui->incrementAmountDoubleSpinBox->value(),
                          ui->distanceSpinBox->value(),
                          ui->valueMaximumDoubleSpinBox->value());
}
void ConductivityEditor::setConductivities(double startVal, double incAmount,
                                           int maxDist, double maxVal) {
  this->getInitial();
  int i = 0;
...
void ConductivityEditor::add(pair<int, int> e, set<pair<int, int>>& next) {
  ...
  if (e.second < 0 || e.first < 0 || e.second >= this->model->rowCount() ||
      e.first >= this->model->columnCount()) {
    return;
  }
...
void ConductivityEditor::setConductivity(pair<int, int> e, double val) {
  for (int i = 0; i < 4; i++) {
    ...
    /*		if((current.count(make_pair(e.first -1, e.second)) == 1 && i ==
       CellUtils::left) || (current.count(make_pair(e.first +1, e.second)) == 1
       && i == CellUtils::right) || (current.count(make_pair(e.first, e.second
       +1)) == 1 && i == CellUtils::bottom) || (current.count(make_pair(e.first,
       e.second -1)) == 1 && i == CellUtils::top))
                    {
                            continue;
            }*/
    // check for overlaping edges in visited
    if ((visited.count(make_pair(e.first - 1, e.second)) == 1 &&
         i == CellUtils::left) ||
        (visited.count(make_pair(e.first + 1, e.second)) == 1 &&
         i == CellUtils::right) ||
        (visited.count(make_pair(e.first, e.second + 1)) == 1 &&
         i == CellUtils::bottom) ||
        (visited.count(make_pair(e.first, e.second - 1)) == 1 &&
         i == CellUtils::top)) {
      continue;
    ...
    this->model->setData(
        this->model->index(e.second, e.first, QModelIndex()).child(1, i), val);
  }
}

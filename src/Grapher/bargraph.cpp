...
barGraph::barGraph(QString name, double value, QString var, QDir saveDir,
                   QWidget* parent)
    : QWidget(parent), ui(new Ui::BarGraph) {
  ui->setupUi(this);
...
void barGraph::Initialize() {
  ui->plot->plotLayout()->addElement(
      1, 0,
      new QCPTextElement(ui->plot, var + " (" + this->unitsMap[var] + ")"));
...
void barGraph::on_loadOtherTrial_clicked() {
  ...
  for (; it != names.end() && iv != values.end(); it++, iv++) {
    if (aspect == *it) {
      bool ok;
      QString name =
          QInputDialog::getText(this, tr("Other Simulation Name"), tr("Name:"),
                                QLineEdit::Normal, *it, &ok);
      if (!ok) {
        name = *it;
      }
      labels.append(name);
...
void barGraph::newBar(bar& newBar) {
  ...
  //    (2007)"); ui->plot->yAxis->grid()->setSubGridVisible(true);

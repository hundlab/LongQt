...
GpbAtrialWT::GpbAtrialWT()
{
    this->Initialize();
}

GpbAtrialWT::GpbAtrialWT(GpbAtrialWT& toCopy) : Cell(toCopy) {
    this->Initialize(); 
...
void GpbAtrialWT::updateInal()
{
	...
	fiNalNP = 1- fiNalP;
	iNal = 	fiNalP * iNalP + fiNalNP * iNalNP;
...
void GpbAtrialWT::updateSRFlux(){

//SR fluxes
	double dRyrr, dRyro, dRyri;
	double maxsr = 15.0;
	double minsr = 1.0;
	double kCasr, kOsrca, kIsrca; //change these
	double ec50SR = 0.45;
	 double koCa = 10.0;  //changed for simulations
	double kiCa =0.5; //change this
	double kim = 0.005; //change this
	double kom = 0.06; //change this
	double ks = 25.0; //change this
	double alpha, beta;
	double kmf = 2.5*0.000246; //change this
	...
	//////////// SD
	//double RyRtot = 382.6; //umol/ L dyad
	//double RyRP = 1; //for S2814D, all channels are phosphorylated
	
	//double RyRratio = RyRP*382.6/RyRtot;

	//double kleak = 1/3 + (10/3)*RyRratio;
	//double koRyRCKII = (20/3)*(RyRratio)-(1/3);
	//double koRyR = koRyRCKII - 1;
	///////////

	kCasr = maxsr -((maxsr-minsr)/(1.0+pow((ec50SR/caSr),2.5)));
	//kOsrca = koRyR*koCa/kCasr; //SD	
	kOsrca = koCa/kCasr; //WT
	kIsrca = kiCa*kCasr;
	...
	Jsrleak =0.000005348*(caSr-cajI); //WT
	//Jsrleak =kleak*ksrleak*(caSr-cajI); //SD
	
	
}
void GpbAtrialWT::updatecytobuff(){
...
void GpbAtrialWT::updateJSLbuff(){
...
// External stimulus.
int GpbAtrialWT::externalStim(double stimval) {
    iTot = iTot + stimval;
    return 1;
}
// Create map for easy retrieval of variable values.
void GpbAtrialWT::makemap()
{
  vars["vOld"]=&vOld;
  vars["t"]=&t;
  vars["dVdt"]=&dVdt;
...
const char *GpbAtrialWT::type() const
{
